...
echo; echo "===== kernel log: vga_switcheroo / gmux / apple / i915 / nvidia registration ====="
# journal first: the dmesg ring buffer rotates on long uptimes and loses early boot
( journalctl -b -k --no-pager 2>/dev/null || dmesg ) | grep -iE 'switcheroo|gmux|apple|i915|nvidia' | tail -60
...
for pci in 0000:00:02.0 0000:01:00.0; do
  drv=$(basename "$(readlink -f "/sys/bus/pci/devices/$pci/driver" 2>/dev/null)" 2>/dev/null)
  echo "$pci -> driver: ${drv:-none}  power/runtime_status: $(cat "/sys/bus/pci/devices/$pci/power/runtime_status" 2>/dev/null)"
done

echo; echo "===== apple_gmux info ====="
ls -la /sys/module/apple_gmux/ 2>/dev/null
( journalctl -b -k --no-pager 2>/dev/null || dmesg ) | grep -i gmux
