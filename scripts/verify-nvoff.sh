...
DISPLAY=:0 glxinfo 2>/dev/null | grep -iE 'OpenGL renderer' || echo "  (glxinfo unavailable)"
echo
echo "===== panel connector (want eDP on INTEL card) ====="
for s in /sys/class/drm/card*-eDP-*/status; do d=$(dirname "$s"); real=$(readlink -f "$d"); case "$real" in *00:02.0*) g=INTEL;; *01:00.0*) g=NVIDIA;; *) g=?;; esac; echo "  $(basename $d): $(cat $s) -> $g"; done
echo
